...
//初始化全局顶点
FVector2D UNoiseTool::GlobalVertex[4] = {};
FVector UNoiseTool::GlobalVertex3D[3] = {};

//初始化全局偏移
FVector2D UNoiseTool::GlobalOffset = FVector2D::ZeroVector;
FVector UNoiseTool::GlobalOffset3D = FVector::ZeroVector;

//初始化全局种子
int32 UNoiseTool::GlobalSeed = 107;

int32 UNoiseTool::hash11(int32 position)
{
...
int32 UNoiseTool::hash11WithGlobalSeed(int32 position){
	const uint32 BIT_NOISE1 = 0x85297A4D;
	const uint32 BIT_NOISE2 = 0x68E31DA4;
	const uint32 BIT_NOISE3 = 0x1B56C4E9;
...
FVector2D UNoiseTool::hash22(FVector2D position2D)
{
	FVector2D v(
...
int32 UNoiseTool::hash21(FVector2D position2D)
{
	return hash11(0x651A6BE1 * (int32)position2D.X + (int32)position2D.Y)%1024;
}

FVector UNoiseTool::hash33(FVector position)
{
	FVector v(
...
float UNoiseTool::perlinNoise(FVector2D p)
{
	p = GlobalOffset + p;
...
//TODO
float UNoiseTool::valueNoise(FVector2D position2D)
{
...
float UNoiseTool::simplexNoise2D(FVector2D p)
{
  p = p + GlobalOffset;
...
//TODO
float UNoiseTool::simplexNoise(FVector p){
 	p = p + GlobalOffset3D;
...
void UNoiseTool::prehandlePerlinNoise(FVector2D position2D, int32 crystalSize){
	FVector2D pi = FVector2D(floor(position2D.X / crystalSize), floor(position2D.Y / crystalSize));
	FVector2D vertex[4] = { {pi.X,pi.Y},{pi.X + 1,pi.Y},{pi.X,pi.Y + 1},{pi.X + 1,pi.Y + 1} };

	for(int32 i = 0;i<4;++i)
	GlobalVertex[i]=UNoiseTool::hash22(vertex[i]);
	
	GlobalOffset = (position2D-pi*crystalSize)/crystalSize;
}

//TODO
void UNoiseTool::prehandleValueNoise(FVector2D position2D, int32 crystalSize){

}

void UNoiseTool::prehandleSimplexNoise2D(FVector2D position2D, int32 crystalSize){
	GlobalOffset = position2D / crystalSize;
}

void UNoiseTool::prehandleSimplexNoise(FVector position,int32 crystalSize){
	GlobalOffset3D = position / crystalSize;
}

//二阶bezier曲线 t应[0.0f~1.0f]
FVector2D UNoiseTool::bezier2(FVector2D p0,FVector2D p1,FVector2D p2,float t){
	return ((1-t)*(1-t)*p0+t*(1-t)*p1+t*t*p2);
}

//三阶bezier曲线 t应[0.0f~1.0f]
FVector2D UNoiseTool::bezier(FVector2D p0,FVector2D p1,FVector2D p2,FVector2D p3,float t){
	return bezier2(p0,p1,p2,t)*(1-t)+bezier2(p1,p2,p3,t)*t;
}

//三维坐标转一维坐标（压缩坐标）
uint64 UNoiseTool::Index(int32 x,int32 y,int32 z){
...
//一维坐标转三维坐标（解压坐标）
FVector UNoiseTool::UnIndex(uint64 index){
	const int32 offset = 16384;
	return FVector(
